...
-- Repeat the same process as before with DR_NO. Check if all reccords in DATE_OCC show a time of 0:00. Then, change the format to DATE.
SELECT DISTINCT DATE_OCC
FROM crime_clean
WHERE DATE_OCC NOT LIKE '%0:00';-- ALL RECCORDS SHOW 0:00

-- Clean DATE_OCC to reflect the YYYY/MM/DD format with new column(DateOfCrime). Drop time because they are all 0:00.
...
ALTER TABLE crime_clean
ALTER COLUMN TimeOfCrime TIME(0);
...
-- NULLs have been checked for and range has been validated. 
-- The time range that this dataset covers is January 1, 2020 to March 28, 2025 for ReportDate.
-- The time range that this dataset covers is January 1, 2020 to March 27, 2025 for DateOfCrime.